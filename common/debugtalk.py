...
class DebugTalk:
    ...
    #不用了 直接改了specification_yaml里面的cookie处理 加上了兼容dic类型
    # def get_pm_token(self, node_name):
    #     token = self.read.get_extract_yaml(node_name)
    #     pmToken = 'pmToken=' + token
    #     return pmToken
    ...
    def format_dict_params(self, data, order="asc"):
        """
        将字典按 key 排序并拼接字符串，包含异常处理和类型兼容
        """
        try:
            # 确定排序顺序
            is_reverse = (str(order).lower() == "desc")

            # 获取所有 key 并排序（如果 data 不是字典，此处会抛出 AttributeError）
            sorted_keys = sorted(data.keys(), reverse=is_reverse)

            pairs = []
            for k in sorted_keys:
                v = data[k]
                # None 转为空字符串，其余转为 str
