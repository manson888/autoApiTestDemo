...
class RequestBase:
    ...
    def specification_yaml(self, base_info, test_case):
        """
        接口请求处理基本方法 可以在这里手动切换环境 修改host对应的环境就行
        envi 本地服务 dev 测试站 prod线上
        ...
        try:
            ...
            if base_info.get('cookies') is not None:
                # 兼容字典格式的值
                raw_cookie = self.replace_load(base_info['cookies'])
                try:
                    cookie_obj = eval(raw_cookie)
                except Exception:
                    cookie_obj = raw_cookie
                # 如果是字典格式 转化成值的格式为："key=value"
                if isinstance(cookie_obj, dict):
                    cookie = '; '.join(f"{k}={v}" for k, v in cookie_obj.items())
                else:
                    cookie = cookie_obj
